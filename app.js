...
let supabaseClient=null,isSignup=false;
...
function openAuth(){modal.classList.remove("hidden");$("auth-title").textContent=isSignup?"خۆتۆمارکردن":"چوونەژوورەوە";$("auth-submit").textContent=isSignup?"خۆتۆمارکردن":"چوونەژوورەوە";$("name-wrap").classList.toggle("hidden",!isSignup);$("name").required=isSignup;$("password").autocomplete=isSignup?"new-password":"current-password";$("auth-message").textContent=""}
$("toggle-auth").onclick=()=>{isSignup=!isSignup;openAuth();};
$("auth-form").onsubmit=async e=>{e.preventDefault();const msg=$("auth-message");if(!supabaseClient){msg.textContent="سیستەمی ئەکاونت هێشتا ڕێکنەخراوە. لە هەنگاوی دواتردا Supabase پەیوەندی پێدەدەین.";return}try{const email=$("email").value.trim(),password=$("password").value;if(isSignup){const name=$("name").value.trim();const {data,error}=await supabaseClient.auth.signUp({email,password,options:{data:{full_name:name},emailRedirectTo:location.origin}});if(error)throw error;msg.textContent=data.session?"خۆتۆمارکردن سەرکەوتوو بوو.":"ئیمەیڵی پشتڕاستکردنەوە بۆت نێردرا."}else{const {error}=await supabaseClient.auth.signInWithPassword({email,password});if(error)throw error;msg.textContent="بەخێربێیت 👋";modal.classList.add("hidden");updateAccount()}}catch(err){msg.textContent="هەڵە: "+(err.message||"نەتوانرا کارەکە تەواو بکرێت")}};
